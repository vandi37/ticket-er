...
func Start() string {
	return `Я бот билетёр /help чтобы узнать команды`
}
func Help() string {
	return Escape(`Мои команды:
	
Кошелёк: просмотр баланса
Начислить: начислить билеты (только для админов)
Забрать: забрать билеты (только для админов)
Передать: передать билеты`)
}
